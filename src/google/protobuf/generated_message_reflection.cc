...
bool Reflection::IsRepeatedFieldEmpty(const Message& message,
                                      const FieldDescriptor* field) const {
  USAGE_CHECK_MESSAGE(IsRepeatedFieldEmpty, &message);
  USAGE_CHECK_MESSAGE_TYPE(IsRepeatedFieldEmpty);
  USAGE_CHECK_REPEATED(IsRepeatedFieldEmpty);
...
bool Reflection::IsFieldNonEmpty(const Message& message,
                                 const FieldDescriptor* field) const {
  ...
  if (field->is_repeated() || field->is_map()) {
    ...
    return !IsRepeatedFieldEmpty(message, field);
  }
...
bool Reflection::IsFieldPresentGivenHasbits(const Message& message,
  ...
  return IsFieldNonEmpty(message, field);
}
